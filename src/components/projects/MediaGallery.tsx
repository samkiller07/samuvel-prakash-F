  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {media.map((item, idx) => {
          const resolvedUrl = storageService.resolveStorageUrl(item.url) || item.url;
          return (
            <div
              key={item.id || idx}
              onClick={() => setSelectedMedia({ ...item, url: resolvedUrl })}
              className="group relative bg-hud-panel border border-hud-border hover:border-hud-green/60 rounded-sm overflow-hidden cursor-pointer transition-all duration-200 select-none"
            >
              {/* Thumbnail container */}
